...
class Ride {
  ...
  index() {
    ...
    const rel = '/alc/'
    const ttl = `${rel}${indy.dataset.ttl}`

    data.URL = indy.dataset.url || this.getMapUrl(ttl)
...
')
  ...
  elevation() {
    ...
    let legbot  = 'h2'
    let legrite = '24'// pt font
    let larger = ''
    let sty = 'margin:50px;'
    let start = '<div style="margin-right:50px; background-color:white; z-index:100; padding:5 20 5 20; border:1px solid gray; float:right;"><a href="index.html">overview</a></div>'
    if (this.arg('half')) {
      WIDE /= 2
      TALL /= 2
    ...
    for (let i = Math.round(MAXY / legendY); i > 0; i -= 1)
    ...
    for (let i = 1; i <= Math.floor(data.maxX / 10); i += 1)
    ...
    for (let i = SAMPLE_CLIMBED; data.points[i]; i += SAMPLE_CLIMBED) {
      let avgy = 0.0
      for (let j = 0; j < SAMPLE_CLIMBED; j += 1)
    ...
    const CLIMBED_SPAN = 0.21// ~1/5th of a mile -- determined by comparing w/ ciclo
    this.feetClimbed = 0.0
    let prevDist = 0.0
    let n = 0
    let sumY = 0.0
    let prevAvgY = data.points[0][1]
    for (let i = 0; data.points[i]; i += 1) {
      const curDist = data.points[i][0]
    ...
    const mulx = WIDE / data.maxX
    const muly = TALL / MAXY
    ...
    ctx.beginPath()
    ctx.moveTo(0, TALL)
    for (let i = SAMPLE; data.points[i]; i += SAMPLE) {
      let avgx = 0.0
      let avgy = 0.0
      ...
      avgx /= SAMPLE
      avgy /= SAMPLE
      const x = Math.floor(avgx * mulx)
      const y = TALL - Math.round(avgy * muly)
      // log(x,y,avgy)
      ctx.lineTo(x, y)
    }
  ...
  linkify() {
    ...
    let links = bodyobj.getElementsByTagName('a')
    if (!links.length) {
      // hmm!  no links! parse document as a text file, line by line

      let bod = bodyobj.innerHTML
      ...
      for (let i = 0; i < lines.length; i += 1) {
        const name = lines[i].replace(/http:\/\/maps\.google\.com\/maps\?/, '...')
        if (lines[i][0] === 'h')
  ...
  arg(theArgName) {
    ...
    let r = ''
    for (let i = 0; i < sArgs.length; i += 1) {
